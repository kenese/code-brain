...
Also check for an optional `skipCute` flag anywhere in the args (e.g.
`skipCute` or `skipCute=true`). When present, this run should skip creating a
cute test environment — carry that through to step 6.
...
that a session is now creating the plan and will ask clarifying questions
there.
